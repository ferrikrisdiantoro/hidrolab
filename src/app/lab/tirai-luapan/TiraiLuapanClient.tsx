...
function notice(
  ...
  if (lang === "en")
    return `At a distance of one head downstream of the crest the nappe has dropped exactly half a head, which is a number that falls straight out of the equation and is marked on the drawing. Over that same distance the projectile path has already fallen ${fmt(beda, 4)} m further. Raise the head, currently ${fmt(h, 3)} m against a crest height of ${fmt(P, 2)} m, and watch the two paths separate faster: the faster the water leaves the crest, the flatter the real nappe becomes relative to a free fall.`;
  return `Pada jarak satu kali tinggi muka air di hilir mercu, tirai sudah turun tepat setengahnya, dan angka itu jatuh langsung dari persamaannya serta ditandai pada gambar. Sepanjang jarak yang sama, lintasan peluru sudah turun ${fmt(beda, 4)} m lebih jauh. Naikkan tinggi muka airnya, sekarang ${fmt(h, 3)} m terhadap tinggi mercu ${fmt(P, 2)} m, lalu perhatikan kedua lintasan memisah lebih cepat: makin kencang air meninggalkan mercu, makin landai tirai sesungguhnya dibanding jatuh bebas.`;
}
